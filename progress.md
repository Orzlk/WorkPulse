...
## 2026-08-25 Flomo 图片附件导入

- 按已确认设计扩展 Flomo HTML 解析模型，保留图片的相对路径、alt 文本和正文位置；文件区图片会补回 Markdown 图片引用。
- 导入时将 PNG/JPG/JPEG/GIF/WebP 复制到 `userData/attachments`，正文引用改写为 `workpulse-attachment://` 安全协议；音视频、缺失、越界和不支持格式逐项跳过并统计。
- IPC、Preload 和工作日志导入提示已增加“已导入图片附件/未导入附件”信息；更新中英文 README。
- 当前已通过 Flomo 解析、导入集成、附件存储安全和 Preload 契约定向测试；全量测试 43 个文件/199 个测试、类型检查和 Electron 生产构建均通过。

## 2026-08-25 工作区信息架构与 Tab 统一

- 顶部导航按领域合并为“记录 / 任务 / 项目 / 复盘”，保留日志、收件箱、看板、项目、仓库、报告、统计的内部页面和搜索深链。
- 记录、项目、复盘页面分别增加统一的二级导航；仓库、统计、收件箱可直接回到同领域的主页面。
- 新增共享 `WorkspacePageHeader` 和 `WorkspaceSectionTabs`，所有工作区页面统一使用面包屑、标题区和选中态。
- 活动导航和筛选选中态统一使用橙色选择语义，任务状态和正文引用颜色保持独立。
- 新增信息架构回归测试；全量测试 47 个文件、212 个测试通过，类型检查、生产构建和 `git diff --check` 通过。

## 2026-08-25 AI 报告默认提示词与输出模板

- 将中英文默认提示词升级为面向团队汇报的专业版本：按项目归类，提炼核心功能、缺陷、成果、风险和后续事项，并明确 Git 记录处理、状态判断和禁止虚构事实的规则。
- 将中英文默认输出模板统一为“总体进展 → 项目进展 → 关键成果 → 进行中与下一步”，没有证据的字段和章节直接省略，不再默认生成缺乏依据的计划内容。
- 周报/月报快照生成链路现在同步使用默认或用户自定义的提示词与模板；设置页重置、语言切换和已有自定义配置的保留逻辑不变。
- 新增默认配置回归测试；`npm test`：48 个测试文件、215 个测试通过；`npm run typecheck`、`npm run build`、`git diff --check` 均通过。
- 本轮改动保持未提交，未执行 `git add`、`git commit` 或 `git push`。

## 2026-08-24 P0-P2 AI、附件与 Inbox MVP

- 报告生成新增 SSE 增量解析、普通 JSON 降级、真实阶段事件、主进程 IPC 流和按请求取消。
- 日志新建输入框与独立编辑窗口支持文件选择、剪贴板粘贴和拖拽图片，附件保存于 userData/attachments，并以安全自定义协议引用。
- Inbox 增加最多 20 条批量 AI 建议生成；建议仅写入 `ai_suggestion`，项目和仓库引用经过白名单校验，仍需人工确认整理。
- 先写失败测试，再实现主进程、preload、服务、存储和渲染层改动；新增针对性测试 14 个通过。
- `npm test`：43 个测试文件、196 个测试通过。
- `npm run typecheck`、`npm run build` 通过；本轮功能改动保持未提交，基线提交为 `f288fe4`。
...
- 确认项目、仓库、标签归属选择也属于完整日志输入区域，搜索和导入导出工具栏保持在卡片外。
- 将归属选择控件纳入 `.quick-entry` 卡片，卡片内部按正文、错误提示、归属选择、工具栏组织视觉层级。
- 桌面端归属控件使用三列布局，窄屏自动切换为单列，避免输入区域产生横向滚动。
- 保留标签高亮、项目/标签联想、回车换行和点击按钮提交等既有交互。
- 先验证回归测试失败，再完成实现；最终全量测试 34 个文件、172 个测试通过，类型检查和生产构建通过。
- 本轮未执行 `git add`、`git commit` 或 `git push`。

## 2026-08-26 0.0.1 发布前整理完成

- 应用版本和 lockfile 已统一为 `0.0.1`。
- GitHub 在线更新的自动检查、手动检查和安装入口已暂时停用；保留现有 updater 实现和 IPC 契约，后续可恢复。数据清理等产品安全确认保持不变。
- README、发现记录、TODO 和任务计划已同步当前 v1 预发布数据库、图片附件导入和发布前剩余事项；Release workflow 已加入完整测试步骤。
- 验证结果：57 个测试文件通过，246 个测试通过、7 个旧迁移链测试跳过；类型检查、生产构建和 `git diff --check` 均通过。
- 生产构建仍提示 `ai.ts` 同时被静态和动态导入，但不影响构建结果。
- 未执行 `git add`、`git commit` 或 `git push`。

## 2026-08-24 日志输入卡片圆角修复

- 用户反馈输入区域仍是矩形；检查最终 `.quick-entry` 样式后确认此前仅设置边框和阴影，没有设置 `border-radius`。
- 新增圆角回归测试，先验证旧样式失败，再增加 `border-radius: 16px`，保持与日志内容卡片一致。
- 最终定向测试 6 个通过；全量测试 34 个文件、173 个测试通过；类型检查、生产构建和 `git diff --check` 通过。
- 本轮未执行 `git add`、`git commit` 或 `git push`。

## 2026-08-24 Flomo 整体视觉风格

- 将浅色主题切换为 Flomo 风格的浅灰画布、白色内容卡片、柔和灰色边框、轻量阴影、绿色主操作和蓝色标签/项目引用。
- 顶部导航改为简洁的无衬线布局，移除旧的金色边线与菱形装饰；页面标题、日期标题和弹层统一为轻量卡片视觉。
- 日志输入区、日志卡片、收件箱、项目卡片、Git 仓库卡片、搜索框和弹窗统一圆角与悬停态，悬停不改变布局尺寸。
- 保留已有输入、标签联想、项目关联、日志编辑、Git 管理、移动端布局和深色模式逻辑，仅调整视觉层。
- 先执行失败测试再实现；定向测试 7 个通过，全量测试 34 个文件、174 个测试通过。
- `npm run typecheck`、`npm run build`、`git diff --check` 均通过；未执行 `git add`、`git commit` 或 `git push`。

## 2026-08-24 编辑日志联想与菜单栏隐藏

- 修复编辑日志页面不支持 `#` / `@` 自动联想的问题，编辑页现在复用标签高亮输入层和项目/多级标签候选菜单。
- 支持输入时筛选、鼠标选择、ArrowUp/ArrowDown、Enter 确认和 Escape 关闭；选择 `@项目` 会同步更新项目归属，正文引用仍保留并参与标签提取。
- 主窗口补充 `autoHideMenuBar: true`，与已有编辑子窗口保持一致，隐藏 Electron 原生菜单栏；应用内导航仍保留。
- 先执行失败测试再实现；定向测试 3 个文件、12 个测试通过，全量测试 36 个文件、176 个测试通过。
- `npm run typecheck`、`npm run build` 均通过；未执行 `git add`、`git commit` 或 `git push`。

## 2026-08-24 编辑窗口正文与布局修复

- 根据用户截图追踪到正文没有丢失，`worklog:get` 仍返回完整 `content`；实际问题是 `TagHighlightTextarea` 根节点未参与父容器伸缩，内部 textarea 塌缩为单行高度，正文被滚动区域裁掉。
- 将 `.worklog-editor-content-shell` 设为纵向 flex，并让 `.worklog-editor-composer` 使用 `flex: 1 1 auto`，恢复多行正文显示和正确编辑区布局。
- 先执行失败测试再实现；最终全量测试 36 个文件、177 个测试通过。
- `npm run typecheck`、`npm run build`、`git diff --check` 均通过；未执行 `git add`、`git commit` 或 `git push`。

## 2026-08-24 联想菜单跟随光标

- 新增日志和编辑日志的 `#` / `@` 候选菜单不再固定在输入框底部，改为根据 textarea 光标位置动态定位，体验接近 Flomo。
- 增加共享光标测量工具，兼容多行换行、输入框滚动和标签高亮层；菜单靠近底部时自动显示到光标上方，靠近右侧时自动向左收敛。
- 先执行失败测试再实现；定向测试 3 个文件、15 个测试通过，全量测试 36 个文件、178 个测试通过。
- `npm run typecheck`、`npm run build` 均通过；未执行 `git add`、`git commit` 或 `git push`。

## 2026-08-24 区分 # 与 @ 高亮颜色

- 输入框新增独立的项目引用高亮类型：`#标签` 保持现有蓝色，`@项目` 使用绿色项目色；正文中的可点击项目引用和项目徽标同步调整。
- 邮箱等非项目形式的 `@` 文本不会被输入高亮规则误识别，标签和项目引用继续保留正文内容。
- 先执行失败测试再实现；定向测试 2 个文件、19 个测试通过，全量测试 36 个文件、180 个测试通过。
- `npm run typecheck`、`npm run build`、`git diff --check` 均通过；未执行 `git add`、`git commit` 或 `git push`。

## 2026-08-24 报告范围默认全选

- 报告页项目范围和仓库范围新增“全部”选项，空范围继续表示不过滤，默认状态明确高亮。
- 选择具体项目或仓库后切换为指定范围；点击“全部”可恢复全选，项目上下文同步恢复为空。
- 周报和月报继续按现有自然周期计算：周一至周日、自然月；切换类型时使用最近完整周期。
- 先执行失败测试再实现；定向测试 2 个文件、9 个测试通过，全量测试 37 个文件、182 个测试通过。
- `npm run typecheck` 通过；未执行 `git add`、`git commit` 或 `git push`。
## 2026-08-26 数据库迁移压缩

- 用户确认未发布阶段直接重置数据库版本为 1，并清空后重新导入。
- 当前阶段：准备将现有最终迁移步骤合并为新的 v1 初始化迁移；未执行 Git 提交。
## 2026-08-26 数据库迁移压缩完成

- 将运行时数据库迁移入口压缩为 `CURRENT_SCHEMA_VERSION = 1` 和单一 `001_initial_schema`，初始化时仍完整创建当前最终结构，包括 FTS5 搜索、项目/标签、Git、报告和看板表。
- 同步更新数据库导入导出、迁移、看板、阶段验收及搜索索引测试；旧 v2～v14 历史升级链测试不再作为未发布版本的兼容目标。
- 验证结果：54 个测试文件通过，233 个测试通过、7 个旧迁移链测试跳过；类型检查、生产构建、`git diff --check` 均通过。
- 未执行 Git 提交。
## 2026-08-26 笔记数据管理入口整理完成

- 将笔记页的 Flomo/CSV/Markdown 导入与 CSV/Markdown 导出入口移到「设置 → 数据管理」的独立卡片。
- 笔记页仅保留记录、搜索、筛选等高频操作；设置页导入成功后保留 Toast 反馈，下一次进入笔记页会重新加载数据。
- 新增中英文文案和入口位置回归测试。
- 验证结果：55 个测试文件通过，235 个测试通过、7 个旧迁移链测试跳过；类型检查、生产构建、`git diff --check` 均通过。
- 未执行 Git 提交。
## 2026-08-26 Flomo 图片显示修复完成

- 确认导入内容和图片文件已正确保存，问题位于 Markdown 渲染层的自定义协议过滤。
- 为 `InteractiveMarkdown` 增加安全 URL 转换器，仅允许本地附件协议通过，普通 URL 仍使用 react-markdown 默认清理逻辑。
- 验证结果：相关测试 11 个通过；全量测试 55 个文件、236 个测试通过、7 个旧迁移链测试跳过；类型检查和生产构建通过。
- 未执行 Git 提交。
## 2026-08-26 图片按钮文案修复完成

- 将新建笔记和编辑窗口的图片按钮统一改为“插入图片 / Insert image”，同步更新 title 和 aria-label。
- 拖拽、粘贴、文件选择及附件保存逻辑保持不变。
- 验证结果：图片附件回归测试 5 个通过，类型检查、生产构建和 `git diff --check` 通过。
- 未执行 Git 提交。

## 2026-08-26 UI 视觉系统统一启动

- 用户确认按 UI 审计建议实施，方向为 Flomo 风格的安静工作台。
- 已完成只读审计：确认全局 CSS 多套规则叠加、Settings 页面 Tailwind 颜色绕过 Token、页面导航层级重复和控件尺寸/圆角不统一。
- 本轮计划保留业务逻辑和数据结构，先用失败测试锁定统一 Token、公共控件和页面壳层，再实施逐页迁移。

## 2026-08-26 UI 视觉系统统一完成

- 新增 `src/renderer/src/styles/ui-system.css`，建立浅色/深色语义色、控件高度、圆角、阴影、焦点态、动效和危险/选中状态 Token。
- 将旧页面语义类映射到统一系统，统一顶部导航、二级 Tab、记录侧栏、笔记卡片、输入卡片、项目/仓库卡片、弹层、空状态和设置卡片。
- 顶部导航使用绿色主状态，标签/项目保留独立颜色，筛选选中状态使用橙色；Settings 中原有 Tailwind zinc/blue 样式通过兼容映射纳入同一语义系统。
- `App.tsx` 和 `WorkspaceSectionTabs.tsx` 接入公共 UI 类名，UI 系统在 `main.tsx` 中于历史 CSS 之后加载。
- 验证结果：UI 定向测试 4 个通过；全量测试 56 个文件、241 个通过、7 个跳过；类型检查、生产构建和 `git diff --check` 通过。
- 授权环境 `npm run dev` 已成功启动 Vite 和 Electron；沙箱直接启动仍会因 esbuild `spawn EPERM` 失败，属于执行环境限制。
- 未执行 `git add`、`git commit` 或 `git push`。
## 2026-08-26 控件边框全面审计启动

- 按控件状态检查报告范围、设置、看板、笔记和项目/仓库页面的选中态、主操作态、普通态及焦点态。
- 已定位报告范围 Picker 的深色边框来源，下一步先补充失败回归测试，再统一语义类和兼容映射，并搜索确认没有遗漏的黑色控件边框。
## 2026-08-26 控件边框全面审计完成

- 报告范围 Picker 改为 `ui-scope-option`，选中态使用橙色选择语义，全部项目/全部仓库与具体范围选项视觉一致。
- 设置页浅色/深色/跟随系统改为 `ui-choice-option`，移除主题按钮自身的黑色边框类。
- UI 系统增加旧式 `border-zinc-900` / `dark:border-zinc-100` 的安全兼容映射，避免尚未迁移的控件产生黑边；同时保留普通边框使用中性 Token。
- 已完成源码审计：除兼容映射和回归断言外未发现黑色边框类；待完成全量测试、类型检查、构建和差异检查。
## 2026-08-26 控件边框审计验证完成

- `npm test`：56 个测试文件通过，243 个测试通过，7 个旧迁移链测试跳过。
- `npm run typecheck`：通过。
- `npm run build`：通过；仅保留既有的 AI 动态/静态导入提示。
- `git diff --check`：通过。
- 本轮未执行 `git add`、`git commit` 或 `git push`。
