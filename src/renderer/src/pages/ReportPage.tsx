...
import { buildReportExportName, getLatestCompleteReportAnchor, getReportGenerationError, type WorkflowReportType } from '../lib/reportWorkflow'
import { useI18n } from '../stores/languageStore'
import { useProjectStore } from '../stores/projectStore'
import { useRepositoryStore } from '../stores/repositoryStore'

type Status = 'idle' | 'no_key' | 'generating' | 'success' | 'error' | 'no_data'
type Stage = 'idle' | 'reading' | 'git' | 'grouping' | 'generating' | 'saving' | 'failed'

interface Report {
  public_id: string; type: string; project_scope: string[]; repository_scope: string[]; content: string; version: number
  status: 'generating' | 'ready' | 'error'; error_message: string | null; generated_at: string | null
...
const toggle = (ids: string[], id: string): string[] => ids.includes(id) ? ids.filter((value) => value !== id) : [...ids, id]
...
function ReportPage({ projectId, onProjectChange, onOpenInbox }: Props): JSX.Element {
  ...
  useEffect(() => { setProjectIds(projectId ? [projectId] : []) }, [projectId])
  useEffect(() => {
    let closed = false
  ...
  const generate = async (): Promise<void> => {
    ...
    try {
      const report = await window.api.report.generate(request) as Report
      clearTimers(); setStage('saving'); setActive(report); setContent(report.content); setStatus(report.content ? 'success' : 'no_data'); setStage('idle'); await loadHistory()
    } catch (cause) {
    ...
    {isHistory ? <HistoryHeader report={viewing} onBack={() => { setViewing(null); setActive(null); setContent(''); setStatus('idle'); setEditing(false) }} t={t} timeZone={timeZone} /> : <>
      <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          ...
          <div className="space-y-4"><Picker label={t('report.projectScope')} items={projects} ids={projectIds} allLabel={t('report.allProjects')} onToggle={(id) => { const next = toggle(projectIds, id); setProjectIds(next); onProjectChange(next.length === 1 ? next[0] : null) }} /><Picker label={t('report.repositoryScope')} items={repositories} ids={repositoryIds} allLabel={t('report.allRepositories')} onToggle={(id) => setRepositoryIds(toggle(repositoryIds, id))} /></div>
        </div>
      </section>
