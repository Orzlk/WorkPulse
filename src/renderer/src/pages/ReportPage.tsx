...
import { buildReportExportName, getHistoryReportState, getLatestCompleteReportAnchor, getReportGenerationError, getRetryReportRequest, hasReportPreviewData, toggleReportScope, type WorkflowReportType } from '../lib/reportWorkflow'
    ...
    {isHistory ? <HistoryHeader report={viewing} onBack={() => { setViewing(null); setActive(null); setContent(''); setStatus('idle'); setEditing(false) }} t={t} timeZone={timeZone} /> : <>
      <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          ...
          <div className="space-y-4"><Picker label={t('report.projectScope')} items={projects} ids={projectIds} allLabel={t('report.allProjects')} onToggle={(id) => { const next = toggleReportScope(projectIds, id); setProjectIds(next); if (next.length <= 1) onProjectChange(next[0] ?? null) }} /><Picker label={t('report.repositoryScope')} items={repositories} ids={repositoryIds} allLabel={t('report.allRepositories')} onToggle={(id) => setRepositoryIds(toggleReportScope(repositoryIds, id))} /></div>
        </div>
      </section>
