    ...
    {isHistory ? <HistoryHeader report={viewing} onBack={leaveHistory} t={t} timeZone={timeZone} /> : <>
      <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          <fieldset><legend className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{t('report.typeLabel')}</legend><div className="mt-2 flex gap-2">{(['weekly', 'monthly'] as const).map((value) => <button key={value} type="button" aria-pressed={type === value} onClick={() => chooseType(value)} className={`ui-choice-option${type === value ? ' is-selected' : ''}`}>{value === 'weekly' ? t('report.weekly') : t('report.monthly')}</button>)}</div><label htmlFor="report-anchor" className="mt-4 block text-sm font-medium text-zinc-800 dark:text-zinc-200">{t('report.anchorDate')}</label><div className="mt-2 flex flex-wrap gap-2"><input id="report-anchor" type="date" value={anchorDate} onChange={(event) => setAnchorDate(event.target.value)} className="min-h-10 rounded-lg border border-zinc-300 bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-zinc-400 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100" />{(['current', 'previous'] as const).map((preset) => <button key={preset} type="button" onClick={() => setAnchorDate(getReportPresetAnchor(type, preset, timeZone))} className="min-h-10 rounded-lg px-3 text-sm text-zinc-600 hover:bg-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-400 dark:text-zinc-300 dark:hover:bg-zinc-800">{type === 'weekly' ? (preset === 'current' ? t('report.currentWeek') : t('report.previousWeek')) : (preset === 'current' ? t('report.currentMonth') : t('report.previousMonth'))}</button>)}</div></fieldset>
...
function Picker({ label, items, ids, allLabel, onSelectAll, onToggle }: { label: string; items: Array<{ public_id: string; name: string }>; ids: string[]; allLabel: string; onSelectAll: () => void; onToggle: (id: string) => void }): JSX.Element {
  const allSelected = isReportScopeAll(ids)
  return <fieldset><legend className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{label}</legend><p className="mt-1 text-xs text-zinc-500">{ids.length ? `${ids.length} ${label}` : allLabel}</p><div className="mt-2 flex max-h-28 flex-wrap gap-2 overflow-y-auto"><button type="button" aria-pressed={allSelected} onClick={onSelectAll} className={`ui-scope-option${allSelected ? ' is-selected' : ''}`}>{allLabel}</button>{items.map((item) => { const selected = ids.includes(item.public_id); return <button key={item.public_id} type="button" aria-pressed={selected} onClick={() => onToggle(item.public_id)} className={`ui-scope-option${selected ? ' is-selected' : ''}`}>{item.name}</button> })}</div></fieldset>
}
