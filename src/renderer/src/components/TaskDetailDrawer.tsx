  ...
  return createPortal(<div className="hallmark-app portal-root task-detail-overlay fixed inset-0 bg-black/25" onMouseDown={(event) => event.target === event.currentTarget && requestClose()}>
    <aside ref={drawerRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={t('kanban.taskDetails')} className="absolute right-0 top-0 flex h-full w-full max-w-lg flex-col border-l border-zinc-200 bg-white shadow-2xl dark:border-zinc-700 dark:bg-zinc-900" onMouseDown={(event) => event.stopPropagation()} onKeyDown={(event) => {
      ...
      <div className="flex-1 space-y-4 overflow-y-auto px-5 py-5">
        <label className="block text-xs font-medium text-zinc-500">{t('kanban.taskTitle')}<input value={title} maxLength={200} onChange={(event) => setTitle(event.target.value)} className="mt-1.5 w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-400 dark:border-zinc-700 dark:bg-zinc-800" /></label>
        <label className="block text-xs font-medium text-zinc-500">{t('kanban.descriptionPlaceholder')}<textarea value={description} maxLength={20000} onChange={(event) => setDescription(event.target.value)} rows={5} className="mt-1.5 w-full resize-y rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-400 dark:border-zinc-700 dark:bg-zinc-800" /></label>
    ...
    {discardConfirmOpen && <ConfirmDialog title={t('common.close')} message={t('kanban.discardChangesConfirm')} confirmLabel={t('kanban.discardChanges')} cancelLabel={t('common.cancel')} danger onConfirm={() => { setDiscardConfirmOpen(false); onClose() }} onCancel={() => setDiscardConfirmOpen(false)} />}
  </div>, document.body)
}
